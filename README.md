...
This fork improves upon the work of Tsekho by altering how underwater geometry is rendered. Instead of applying a pink gradient to the under water portion of rock meshes, all submerged geometry (rocks and terrain) recieve contour lines and are tinted a color associated with the depth band they fall into.
...
Completed:
 ...
 Planned:
 - [ ] Modify the countour generation to more closely resemble [Rustard's Improved Map Mod](https://rustard.itch.io/improved-map-mod)
...
Depths deeper than 32.4 aren't really relevant ingame so underwater geometry is not rendered past this depth.

Neither sub can go below 37.4m (equivalent to 30m from the dive officer)

## Removal of depth bands for detailed and simpele variants
In order increase the readability of the large ship drafts (aka beach depths) I cut down on the number of depth bands I rendered from 10 to 4.
...
0 ballast refers to when a submarine has all 3 ballast tanks completely empty of water (full of air). This is when a submarine is most bouyant.
...
This depth is NOT being measured at the bottom of the hull but instead from an invisible sensor located with a unqiue offset from the bottom of the hull for each of the two submarines.
...
There are several depths reported by dive officers that are relevant for both sumbarines. These submarine behaviors are tied to the dive officer reading, not where the bottom or top of the hull is.
...
To determine the "height" of a Trident we placed one trident on the floor of a river and positioned another immediately ontop of it. Then we measured the difference in dive officer readings.

Trident at bottom DO: 12.3m
Trident ontop of bottom Trident DO: 6.4m
...
Consisder that submarine_height = DO_to_hull_top_offset + DO_to_hull_bottom_offset
...
| BMS Longhook | Trident |  0.37m | 4.18m | 3.81m | 
| BMS Bluefin | Nakki |  0.37m | 4.18m | 3.81m | 
| BMS Bluefin | Nakki |  0.37m | 4.18m | 3.81m | 
| BMS Bluefin | Nakki |  0.37m | 4.18m | 3.81m | 
| BMS Bluefin | Nakki |  0.37m | 4.18m | 3.81m | 
...
Knight of Science detailed depth coloring with custom AI ranges, in game view.

<img src="screenshots/tsekho_original.png" alt="tsekho_original" width="700"/>

Example of Tsekho dive alert coloring. Obtained from [Tsekho's website](https://foxbunker.com/map/?x=-4607&y=744&z=11).